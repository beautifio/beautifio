...
export async function saveOnboardingData(
  userId: string,
  data: { status: string; interests: string[]; goals: string[] }
) {
  const client = requireClient();
...
// === CERITA (STORIES) ===
export async function getStories(filters?: { category?: string; userId?: string }) {
  const client = requireClient();
  let query = client
    .from("stories")
    .select("*")
...
export async function likeStory(storyId: string, userId: string) {
  ...
  if (!error) {
    await client.rpc("increment_story_likes", { story_id: storyId });
  }
  return { error };
}
...
export async function unlikeStory(storyId: string, userId: string) {
  ...
  if (!error) {
    await client.rpc("decrement_story_likes", { story_id: storyId });
  }
  return { error };
}
